...
export default function PlanDetailScreen() {
  ...
  const latestDraft = usePlanStore((state) => state.drafts[0]);
  ...
  return (
    <Screen>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Salida #{planId}</Text>
      </View>

      <View style={styles.card}>
        <View style={styles.iconBox}>
          <CalendarClock color={colors.primary} size={34} strokeWidth={2.1} />
        </View>
        <Text style={styles.cardTitle}>
          {latestDraft?.name ?? "Detalle listo para conectar con la API."}
        </Text>
        <Text style={styles.cardText}>
          Aquí vivirán el resumen, participantes, presupuesto y decisiones finales de la noche.
        </Text>
      </View>
    </Screen>
  );
